...
def main() -> None:
    ...
    ax.plot(steps, vals, color="#c8233a", linewidth=2)
    ax.set_xlabel("environment steps", color="#7b748f")
    ax.set_ylabel("mean episode reward", color="#7b748f")
    ax.set_title(f"PPO training, {run_dir.name}", color="#38324a")
    ax.tick_params(colors="#7b748f")
    for spine in ax.spines.values():
        spine.set_color("#e6e2f0")
    ax.grid(color="#ece9f4", linewidth=0.7)
    fig.tight_layout()
